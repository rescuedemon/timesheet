...
const renderBoat = (type: string) => {
  ...
  const testItems = [
    { type: 'sailboat', name: 'Sailboat' },
    { type: 'motorboat', name: 'Motorboat' },
    { type: 'canoe', name: 'Canoe' },
    { type: 'fish', name: 'Fish' },
    { type: 'whale', name: 'Whale' },
    { type: 'shark', name: 'Shark' },
    { type: 'mermaid', name: 'Mermaid' },
    ...
    >
      ...
      <div className="absolute top-4 right-4 z-30">
        <div className="relative">
          ...
          {showTestDropdown && (
            <div className="absolute right-0 mt-2 w-48 bg-gray-900/90 backdrop-blur-sm rounded-lg shadow-lg border border-gray-800/50 max-h-60 overflow-y-auto">
              <div className="p-2">
                <div className="text-xs text-gray-400 mb-2 px-2">Test Items</div>
                {testItems.map(item => (
                  <button
                    key={item.type}
                    className="w-full text-left px-3 py-2 text-sm text-white hover:bg-gray-700/50 rounded transition-colors flex items-center space-x-2"
                    onClick={() => {
                      if (boats.length < 10) {
                        setBoats(prev => [...prev, { 
                          id: Date.now(), 
